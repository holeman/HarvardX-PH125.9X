---
title: "Sun City Decision Tree Model"
...
subtitle: Harvard PH125 Capstone 2
---
...
The general goal of machine learning in prediction is to build models out of the patterns in data to use for making future predictions. To that end, this project concerns the construction of a project-oriented dataset and the building of two such machine learning models.
...
homeownership and rising home values. Accordingly, the project deals in home selection and sale price.
...
The project dataset contains features of the 935 homes sold in Sun City Lincoln Hills from 2019 through 2021. The home features are the entries in the dataset columns, and each row represents a home sold.
...
Overview plot of dataset features shows the categorical and numerical variables in the data. Correlation coefficients are Pearson by default. The skewness in plots can affect machine learning algorithms.
...
Home type(s) selection for modeling. 
...
Refining features selection for the modeling dataset.
...
knitr::include_graphics('G:/000 Tidymodels/SunCity/data/Sun_Villages.jpg')
```
...
Visualization of modeling dataset features.
...
Features correlation reference table.


```{r include=FALSE}
houses_cor <- houses %>% select(where(is.numeric))
correlate(houses_cor)
```


EDA is complete, and on with modeling. 
...
While tidymodels workflows provide for combining models, they appear independently to facilitate model summaries and viewer feedback.
...
Two supervised machine learning goals were pursued in the project.

  1. The Decision Tree classification model sought to predict the home type based on the other feature variables.

  2. The KNN regression model sought to predict home selling price based on dataset features.

Both project models met their goals, and their results are in line with considerations drawn from exploratory data analysis.

As for model processing, Decision Tree preparation requires neither data normalization nor scaling, and KNN modeling requires few assumptions about what the data must look like. Additionally, both models are considered easy to interpret to end users.  The Decision Tree model is intuitive and easy to explain as is the case for the intuitive algorithm of the KNN model with its straightforward interpretation. 

Both models, however, have their disadvantages as well. For a Decision tree, a small change in the data can cause a large change in the structure of the decision tree causing instability. So too for the KNN model.  A handful of points can artificially increase R2. As for the R2 itself, it is a measure of correlation, not accuracy, and can be a deceiving metric. A KNN model may have a strong linear relationship between its predicted values and the observed values, yet the predicted values may not conform to the so-called 45-degree line of agreement. 

Moreover, the models may not predict well beyond the range of values input in the training data. I look forward to put this potential to the test with Sun City house sale prices in 2022.

In closing, I would like to express my appreciation to Professor Rafael Irizarry and the staff for offering this nine-course journey. I also thank my fellow students for making time to provide peer group feedback.
